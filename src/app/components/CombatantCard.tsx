        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              ...
              />
              {isDead && <span className="px-2 py-0.5 bg-red-600 text-white text-xs rounded-full">Dead</span>}
            </div>
      ...
      {combatant.statuses && combatant.statuses.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2 text-xs text-white">
          {combatant.statuses.map((status, i) => (
            <span key={i} className="bg-gray-600 px-2 py-0.5 rounded-full flex items-center gap-1">
              {status}
              <button
                onClick={() => updateStatuses(combatant.statuses!.filter((_, idx) => idx !== i))}
      ...
      <input
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" && e.currentTarget.value.trim() !== "") {
            ...
            if (!combatant.statuses?.includes(newStatus)) {
